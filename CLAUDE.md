# Claude Code Rules for FanCoolo Plugin
...
- Follow WordPress standards; PSR-4 under `FanCoolo\…`; class names match files and directories.
...
- Move/rename `src/utils/FunculoApiClient.js` → `src/utils/api/FanCooloApiClient.js`.
...
- Expose only stable globals/APIs (e.g., `window.FanCoolo.*` or WP hooks). Document versioned changes in `README.md`.
...
- Standardize namespaces under `FanCoolo\{Area}\...` aligned with folders (PSR-4 `FanCoolo\` → `app/`).
...
- Use `FANCOOLO_DEV_MODE=true` in .env for development
...
- Use `FanCoolo\` as the root namespace (not `Marko\FanCoolo\`)
...
- **Steps**:
  1. **Compare Generated Output** (`fancoolo-blocks/[block-name]/`):
...
- **Example**: `BCOMP heading` → Compare both `fancoolo-blocks/heading/` output AND `app/FilesManager/Generators/` templates with core/heading
...
- **Actions**:
  ...
  - Enable `FANCOOLO_DEV_MODE=true` in .env
  - Start file watchers
